#!/bin/bash
#SBATCH --job-name=wind_elev_test
...
#SBATCH --time=2:00:00
#SBATCH --output=logs/topoflow_wind_test_%j.out
#SBATCH --error=logs/topoflow_wind_test_%j.err

# TopoFlow Wind Scanning Test (8 nodes = 64 GPUs)
# Test if code works at smaller scale
...
# MIOpen cache disabled to avoid SQLite locking issues in multi-node
...
srun python -u main_multipollutants.py --config configs/config_wind_test.yaml

echo "Wind Scanning Test completed!"
