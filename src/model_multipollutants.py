...
class MultiPollutantLightningModule(pl.LightningModule):
    ...
    def _create_china_mask(self):
        ...
        try:
            ...
            print(f"# # # # # #  china_mask load failed: {e} # �� fallback rectangle")
    ...
    def _masked_rmse(self, y_pred, y, valid_mask_bool):
        """Calculer RMSE masquée pour les pixels valides."""
    ...
    def training_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            for i, name in enumerate(self.target_variables[:y_pred.size(1)]):
                # Utiliser le masque pour ce polluant spécifique
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Métriques par polluant (optionnel, simple)
        for i, pollutant in enumerate(self.target_variables):
            if i < y_pred.shape[1]:  # Sécurité dimension
    ...
    def on_test_epoch_end(self):
        """Appelé �#  la fin de l'évaluation test."""
        print("\n" + "="*60)
    ...
    def on_fit_end(self):
        """Appelé �#  la fin de l'entraînement pour lancer test automatique."""
        print("\n" + "="*60)
        ...
        try:
            ...
            if hasattr(self.trainer, 'checkpoint_callback') and self.trainer.checkpoint_callback:
                ...
                if checkpoint_dirs:
                    checkpoint_dir = checkpoint_dirs[-1]  # Plus récent
                else:
                    print("# �#  Impossible de trouver le dossier checkpoints")
                    return
            ...
            if best_checkpoint is None:
                ...
                print("# # # # # #  Prise du checkpoint le plus récent")
            
            print(f"# # # � Meilleur checkpoint: {best_checkpoint.name}")
            ...
            for ckpt in checkpoints:
                if ckpt != best_checkpoint:
                    try:
                        ckpt.unlink()
                        print(f"# # # # # # #   Supprimé: {ckpt.name}")
                    except:
                        pass
            
            print(f"# # #  Gardé uniquement: {best_checkpoint.name}")
            ...
            # Créer nouveau trainer pour test (m�# me config GPU)
            import pytorch_lightning as pl
